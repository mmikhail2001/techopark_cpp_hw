...
TEST(TestBasicsFunctional, Test_Before_Begin)
{
	Set<A> set;
	set = {{1, 2}, {3, 4}, {5, 6}};
	auto it = set.begin();
...
TEST(TestBasicsFunctional, Test_Strange)
{
	Set<int> set;
	auto it = set.end();
	++it;
	--it;
	set.insert(55);
	EXPECT_EQ(*(set.find(55)), 55);
}

TEST(TestBasicsFunctional, Test_Iterator_default_ctor)
{
	Set<int> set = {1, 2, 3};
	Set<int>::iterator it;
	it = set.begin();
...
TEST(TestBasicsFunctional, Test_Insert_Erase_Random)
{
	Set<int> set = {4, 6, 2};
	set.insert(1);
	set.insert(3);
...
TEST(TestBasicsFunctional, Test_Compatibility_with_STL_Vector)
{
	...
	int count = std::count_if(set.begin(), set.end(), [](int data) { return data % 2 == 0; });
	EXPECT_EQ(count, 2);
